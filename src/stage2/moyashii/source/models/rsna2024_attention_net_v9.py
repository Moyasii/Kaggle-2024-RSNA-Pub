    ...
    def forward(self,
        ...
        if self.training:
            for condition in self._conditions:
                ...
                slice_logits = sum([self.slice_fcs[condition](dropout(attention_features))
                                   for dropout in self.dropouts]) / len(self.dropouts)  # (batch_size, num_slices, num_classes)
                slice_logits_list.append(slice_logits)

                condition_features = attention_features.mean(dim=1)  # (batch_size, feature_dim)
                logits = sum([self.fcs[condition](dropout(condition_features)) for dropout in self.dropouts]) / \
                    len(self.dropouts)  # (batch_size, num_classes)
                logits_list.append(logits)
            level_logits = sum([self.level_fc(dropout(aggregated_features)) for dropout in self.dropouts]) / len(self.dropouts)
        else:
            for condition in self._conditions:
                attention_features = self.slice_attentions[condition](combined_features)  # (batch_size, num_slices, feature_dim)
                slice_logits = self.slice_fcs[condition](attention_features)  # (batch_size, num_slices, num_classes)
                slice_logits_list.append(slice_logits)
        ...
        slice_logits = torch.stack(slice_logits_list, dim=2)  # (batch_size, num_slices, num_degenerative_conditions, num_classes)
        ...
        if self.training or force_loss_execute:
            losses = self.loss(logits, targets, slice_logits=slice_logits, slice_labels=slice_labels,
                               slice_weights=slice_weights, level_logits=level_logits, level_targets=level_targets)
...
if __name__ == '__main__':
    ...
    model = RSNA2024AttentionNetV9(timm_model={"model_name": "efficientnet_b0", "pretrained": False,
                                               'features_only': False, 'in_chans': 1, 'drop_rate': 0.3, 'drop_path_rate': 0.2},
                                   loss={"name": "RSNA2024Loss", "levels": ["L1/L2"], "level_loss_weight": 0.1, "slice_loss_weight": 1.0},
                                   num_classes=3,)
    model(torch.randn(4, 15, 224, 224),
          torch.randn(4, 15, 224, 224),
          torch.randn(4, 10, 224, 224),
          F.one_hot(torch.randint(0, 3, (4, 5, 1)), num_classes=3).float(),
          F.one_hot(torch.randint(0, 3, (4, 40, 5)), num_classes=3).float(),
          torch.rand(4, 40, 5),
